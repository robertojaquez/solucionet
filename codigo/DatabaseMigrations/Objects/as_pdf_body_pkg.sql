create or replace package body inv_db.as_pdf is
...
--
  ...
  is
    ...
    procedure borders is
    begin
      if (p_txt is not null and p_borders is not null) then 
        if (p_borders in('b','B')) then
          horizontal_line(p_x          => t_x,
                          p_y          => t_y - 1,
                          p_width      => t_len,
                          p_line_width => 0.5,
                          p_line_color => '000000');
        elsif (p_borders in('t','T')) then
          horizontal_line(p_x          => t_x,
                          p_y          => t_y + t_line_height - 5,
                          p_width      => t_len,
                          p_line_width => 0.5,
                          p_line_color => '000000');
        elsif (p_borders in('a','A')) then
          rect(p_x          => t_x - 3,
               p_y          => t_y - 3,
               p_width      => t_len + 3 + 3,
               p_height     => t_line_height - 3,
               p_line_color => '000000',
               p_line_width => 0.1);
        end if;
      end if;
...
--
    ...
    -- String contain LF   (LineFeed Char)
      ...
      write( rtrim( substr( p_txt, 1, t_ind - 1 ), chr(13) ), t_x, t_y, t_line_height, t_start, t_width, p_alignment, k_um, p_borders );
      borders();
      ...
      write( substr( p_txt, t_ind + 1 ), t_start, t_y, t_line_height, t_start, t_width, p_alignment, k_um , p_borders);
      borders();
      return;
    end if;
    ...
    -- Call write recursvely for begin part
      write( substr( p_txt, 1, t_ind - 1 ), t_x, t_y, t_line_height, t_start, t_width, p_alignment, k_um , p_borders);
      t_y := chkNewPage(t_y,t_line_height);
    -- Call write recursvely for remain part
      if t_alignment='J' then
        if str_len(substr( p_txt, t_ind + 1 )) <= t_width then
          write( substr( p_txt, t_ind + 1 ), t_start, t_y, t_line_height, t_start, t_width, 'L', k_um , p_borders);
          borders();
        else
          write( substr( p_txt, t_ind + 1 ), t_start, t_y, t_line_height, t_start, t_width, p_alignment, k_um, p_borders );
          borders();
    ...
    -- Call recursively write for the new line
      write( p_txt, t_start, t_y, t_line_height, t_start, t_width, p_alignment, k_um, p_borders );
    else
      ...
      -- Only one char
        ...
        write( p_txt, t_x, t_y, t_line_height, t_start, t_len, null, k_um, p_borders );
      else
        ...
        write( substr( p_txt, 1, t_ind - 1 ), t_x, t_y, t_line_height, t_start, t_width, p_alignment, k_um, p_borders );
        t_y := chkNewPage(t_y, t_line_height);
        write( substr( p_txt, t_ind ), t_start, t_y, t_line_height, t_start, t_width, p_alignment, k_um, p_borders );
      end if;
    end if;
...
--
    t_img.type := 'png';
    t_img.nr_colors := 4;/*case color_type
                         when 0 then 1
                         when 2 then 3
                         when 3 then 1
                         when 4 then 2
...
--
    dbms_output.put_line('png paint color_type '||color_type);
    dbms_output.put_line('png paint nr_colors '||t_img.nr_colors);
    return t_img;
  end;
--
